...
To set up a Satellite location, you must first create the location, attach hosts to it, and then create the location control plane. Note that in order to use [Red Hat CoreOS](/docs/satellite?topic=satellite-infrastructure-plan) for your worker nodes, you must enable it in your location. For more information, see [Creating a location](/docs/satellite?topic=satellite-locations). 
