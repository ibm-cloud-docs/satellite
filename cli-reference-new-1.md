...
:    Optional. The name of the infrastructure provider to create the {{site.data.keyword.satelliteshort}} location in. Accepted values are `aws`, `azure`, `gcp`<ff-vmware>, and `vmware`</ff-vmware>. If you include this option, you must also include the `--provider-credential` option.
...
```sh
ibmcloud sat mesh ls [--output OUTPUT] [-q]
```
{: pre}
