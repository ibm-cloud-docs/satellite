...
In order to attach Red Hat CoreOS (RHCOS) hosts, you must have Red Hat CoreOS enabled in your location. For more information, see [Creating a location](/docs/satellite?topic=satellite-locations). Note that you can still attach Red Hat Enterprise Linux (RHEL) hosts to a location that is enabled for Red Hat CoreOS.
...
### Attaching RHCOS hosts to an on-premises data center with the console
{: #attach-rhcos-hosts-console}
...
### Attaching RHCOS hosts to an on-premises data center from the CLI
{: #attach-rhcos-hosts-cli}
