...
1.  List your hosts in a location and review the **State**.
    ```
...
1.  Get the location ID where your host is attached, and note the {{site.data.keyword.cloud_notm}} multizone region that the location is managed from.
    ```
...
4.  Reassign your hosts if you continue to have issues.
    ...
    6.  Check that the host is added to your location and **unassigned**.
        ```
        ibmcloud sat host ls --location <location_name_or_ID>
        ```
        {: pre}
    ...
    8.  Check that the host is **assigned** to your cluster. The process might take an hour to complete.
        ```
        ibmcloud sat host ls --location <location_name_or_ID>
        ```
        {: pre}
