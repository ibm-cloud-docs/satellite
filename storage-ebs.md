...
You can use the <staging>[console](#sat-storage-aws-ebs-ui) or [</staging>CLI<staging>](#sat-storage-aws-ebs-cli)</staging> to create an AWS EBS storage configuration in your location and assign the configuration to your clusters to dynamically provision AWS EBS storage for your apps.
{: shortdesc}
...
3. Optional: Follow step 3 in [Removing the AWS EBS storage configuration from the CLI](#aws-ebs-template-rm) to verify that the AWS driver pods and storage classes are removed from your clusters.
